...
def parse_section_from_chunk_id(chunk_id: str) -> str | None:
    match = re.match(
        r"^(?:video|keystrokeData|screenRecording|screenCamera)-([a-z_]+)-\d{13}", chunk_id
    )
    return match.group(1) if match else None


SCREEN_CAMERA_CHUNK_PREFIX = "screenCamera-"


def screen_artifact_type(chunk_id: str) -> Literal["screenRecording", "screenCamera"]:
    
    return (
        "screenCamera" if chunk_id.startswith(SCREEN_CAMERA_CHUNK_PREFIX) else "screenRecording"
    )
...
def build_artifact_registry(
    ...
    for span in screen_spans:
        ...
        records.append(
            ArtifactRecord(
                artifact_id=span.chunk_id,
                artifact_type=screen_artifact_type(span.chunk_id),
                section_id=section_id,
                sequence=span.sequence,
                session_start_ms=span.start_offset_ms,
                session_end_ms=span.end_offset_ms,
...
def build_master_timeline(
    ...
    *,
    ...
    layers = [
        ...
        TimelineLayer(
            type="screenCamera",
            artifacts=[item for item in artifact_registry if item.artifact_type == "screenCamera"],
        ),
    ]
