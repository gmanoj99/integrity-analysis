...
def build_machine_facts(
    ...
    *,
    ...
    combined_artifacts = sum(
        1 for record in timeline.artifact_registry if record.artifact_type == "screenCamera"
    )
    if combined_artifacts:
        limitations.append(
            f"{combined_artifacts} recording chunk(s) capture the webcam inside the screen "
            "video, so their screen and camera observations derive from a single recording "
            "rather than two independent captures."
        )
