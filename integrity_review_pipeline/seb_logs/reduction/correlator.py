...
class SignalCorrelator:
    ...
    def correlate(self) -> list[CorrelatedIncident]:
        ...
        Returns a list of incidents where each incident contains signals
        that occurred within the configured time window of each other.
        """
        if not self._events:
            return []
            ...
            if not current_cluster:
                ...
                if time_diff <= self._time_window:
                    ...
                    if len(current_cluster) > 1:
                        incidents.append(self._create_incident(current_cluster))
