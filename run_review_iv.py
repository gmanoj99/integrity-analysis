...
``integrity_Review/worker/contracts.py``
...
import httpx
from integrity_Review.adapters import (
    GoogleGeminiClient,
...
from integrity_Review.contracts.evidence import EvidenceChunkRef
from integrity_Review.deps import PipelineDeps
...
from integrity_Review.deliberation import rules as _rules
from integrity_Review.deliberation import engine as _delib_engine
from integrity_Review.perception import chunk_job as _chunk_job
from integrity_Review import pipeline as _pipeline
from integrity_Review.pipeline import run_integrity_review
from integrity_Review.timeline import build_master_timeline
from integrity_Review.worker.contracts import (
...
async def run(args: argparse.Namespace) -> int:
    input_paths: list[Path] = list(args.input) or [ROOT / "new_input" / "input.json"]
    ...
    if args.deliberation_model:
        ...
        # pipeline.py does `from .prompts.shared import DELIBERATION_MODEL` and
        # passes that name to the deliberation call, so rebinding
        # DEFAULT_GEMINI_PRO_MODEL alone left the call on the flash default and
        # the flag silently did nothing.
        _pipeline.DELIBERATION_MODEL = args.deliberation_model
        _rules.DELIBERATION_MODEL_VERSION = args.deliberation_model
        # engine.py imported the name by value, so the bundle's provenance
        # kept reporting the flash default even though the pro model ran.
        _delib_engine.DELIBERATION_MODEL_VERSION = args.deliberation_model
        logger.info(
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=Path, default=ROOT / "new_output")
    parser.add_argument(
        "--bundle-name",
